...
public class TSD3ScoreAgent : AbstractSliceHandler
{
	...
	void CheckAboutFallingOutOfView()
	{
		...
		{			
			TSD3ScoreModel.instance.missed++;
		}
	...
	{	
		if(isResultOfSlice)
			TSD3ScoreModel.instance.secondarySlices++;
		else
			TSD3ScoreModel.instance.primarySlices++;
		
		if(!isResultOfSlice)
		{
			for(int i = 0; i < results.Length; i++)
			{
				TSD3ScoreAgent target = results[i].GetComponent<TSD3ScoreAgent>();
